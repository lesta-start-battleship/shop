        ...
        if data.get('success'):
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Создание Purchase после успешного завершения
                    try:
                        item = Product.objects.get(id=transaction.product_id) if transaction.product_id else None
                        chest = Chest.objects.get(id=transaction.chest_id) if transaction.chest_id else None
                        promo = Promotion.objects.get(id=transaction.promotion_id) if transaction.promotion_id else None
